...
The rendering code in each backend is nearly identical and produce almost pixel exact outputs,
...
Similarly, ignoring alpha-cut out effects allows the GPU renderers specifically
to skip using an any hit shader. The any hit shader would be called during BVH traversal,
which in the case that it is hardware accelerated, requires a large amount of back and
forth between the traversal hardware and the shader cores to run the any hit
...
> A large number of light paths typically need to be sampled to produce a noise-free
> image. Path tracing is not restricted to photo-realism, and is the
> core rendering algorithm used in major film renderers, for example: Pixar's Renderman,
> Disney's Hyperion, Autodesk's Arnold, and Weta's Manuka.
> See [Wikipedia](https://en.wikipedia.org/wiki/Path_tracing) for more.

# GPU Ray Tracing with Metal
...
path tracing based renderer. ChameleonRT only performs path tracing, and takes
...
and is a common point of confusion for developers new to these APIs.
...
and continue the secondary rays while filtering out those that do not continue.
...
to be easier to use.
This API streamlining appears to be the case across the rest of the Metal API as well.
...
which is a feature Metal shares with OptiX, which uses CUDA for the device side code.
...
pipeline must individually mark them as used in a loop instead of a single `useHeap` call.
...
This can add some overhead if you have many such bottom level BVHs, for example in scenes with many instances.
...
structure when implementing custom geometries or other effects that happen during intersection
(e.g., alpha cut outs).
Code for managing the SBT setup in DirectX, Vulkan, and OptiX, makes up a significant
portion of my helper code in each backend. Similar work isn't needed by the Metal backend,
which doesn't implement custom geometry or alpha cut out textures.
...
In the end, you may still find yourself implementing an SBT-like model using Metal's new
function pointers and function tables if you need to implement alpha cut out transparency
or custom geometries, or something simpler but a bit like it through how parameters get accessed via
argument buffers, as is the case in ChameleonRT.
In the end though, these are relatively small issues.
...
ISPC for SIMD programming, and TBB for multi-threading.
...
code which is compiled to run in parallel on the CPU's SIMD lanes,
with one program instance per vector lane. It's somewhat like GLSL or HLSL
running on the CPU, where a scalar looking program is executed in parallel using SIMD.
However, ISPC already ships support for NEON and AArch64, and enabling a
...
any specific ties to a specific CPU architecture. All that's needed
here is to build TBB from source for AArch64.

# Benchmarks

With ChameleonRT now running on the M1's CPU and GPU, we're ready to do some
benchmarks and see how the M1 looks in terms of plain ray traversal performance!
I ran these tests on a few different systems that I have access to.
On the CPU side, I have an i7-1165G7 in an XPS 13 (thermals are an issue here),
...
For the benchmarks in ChameleonRT we'll use the two scenes shown below for benchmarking: Sponza and San Miguel.
Sponza is a smaller scene, with 262K triangles, while San Miguel is a decent size for interactive
ray tracing at 9.96M.
...
with thermals during the benchmarks, though that might also happen if it was in a bigger
case with more air flow anyways. I also include my old desktop CPU, the i7-4790K,
in this comparison since it came out in 2014 and scores similar to the i7-1165G7.
...
The i7-1165G7 has AVX512, allowing ISPC to run 16 "programs" in parallel and
Embree to use up to AVX512. The i7-4790K supports AVX2, allowing ISPC to
run 8 "programs" in parallel.
...
just using ISPC's 4-wide target performed best. Embree on the M1 is configured
...
against these systems on these tasks.
...
In these benchmarks I found that the i9-9920X performs best when just using AVX2.
...
<div class="col-12 row mb-2">
    ...
    <div class="col-6">
        ...
        <p>
        <b>Table 5:</b> <i>San Miguel.</i>
        </p>
        </div>
    </div>
    <b>Tables 4 & 5:</b> <i>Extremely unfair benchmark results on Sponza and San Miguel
    using the Embree CPU backend.</i>
</div>
...
To wrap up, we can take a look at the CineBench scores and the performance differences
...
but other "stuff" in CineBench is faster, we could expect higher relative
scores in CineBench than we see on traversal performance alone.
...
with just SSE4 support (4-wide SIMD), which is about 1.6-1.8x slower
than when run with AVX2 (8-wide SIMD).
Even assuming we get just 1.6x faster on the M1 with 8-wide SIMD,
this would put it at ~2.75FPS on Sponza and ~2.06 FPS on San Miguel.
